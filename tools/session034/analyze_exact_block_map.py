#!/usr/bin/env python3
"""Build publication-safe Session 034 exact-block map reports."""

from __future__ import annotations

import argparse
import csv
import json
from pathlib import Path
import tempfile

from phoenix_mmi.binary import BinaryReader
from phoenix_mmi.exact_block_map import (
    analyze_exact_block_map,
    build_public_exact_block_map_report,
    compare_exact_block_seed_control,
    correlate_exact_block_map,
)
from phoenix_mmi.iso9660 import ISO9660Image
from phoenix_mmi.report import write_json


CD1_MEMBER = "MMI_HI/MMI/42/DEFAULT/H2_HI_EU.BIN"
CD3_MEMBER = "MMI_HI/MMI/42/DEFAULT/H2_HI_EU_R1006_SH3_AUDIHI_5.BIN"


def _integer(value: str) -> int:
    return int(value, 0)


def _register(path: Path) -> dict[str, dict[str, str]]:
    with path.open(newline="", encoding="utf-8") as handle:
        return {row["artifact"]: row for row in csv.DictReader(handle)}


def _load_json(path: Path, schema: str) -> dict[str, object]:
    report = json.loads(path.read_text(encoding="utf-8"))
    if report.get("schema") != schema:
        raise ValueError(
            f"unsupported schema in {path}: {report.get('schema')!r}"
        )
    return report


def _verify_iso(image: ISO9660Image, row: dict[str, str]) -> None:
    if image.path.stat().st_size != int(row["size_bytes"]):
        raise ValueError(f"registered size mismatch for {image.path.name}")
    if image.sha256() != row["sha256"].lower():
        raise ValueError(f"registered SHA-256 mismatch for {image.path.name}")


def _disc_report(
    comparison: dict[str, object], *, side: str
) -> dict[str, object]:
    index = 0 if side == "cd1" else 1
    prefix = "left" if side == "cd1" else "right"
    lane_rows = []
    for lane in comparison["lanes"]:
        lane_rows.append(
            {
                "zone_id": lane["zone_id"],
                "zone": lane[f"{prefix}_zone"],
                "window": lane[f"{prefix}_window"],
                "expected_zone_delta": lane["expected_zone_delta"],
                "seed_census": lane["seed_census"],
                "exact_block_count": lane["exact_block_count"],
                "expected_delta_exact_block_count": lane[
                    "expected_delta_exact_block_count"
                ],
                "non_expected_delta_exact_block_count": lane[
                    "non_expected_delta_exact_block_count"
                ],
                "expected_delta_zone_overlap_block_count": lane[
                    "expected_delta_zone_overlap_block_count"
                ],
                "expected_delta_zone_covering_block_count": lane[
                    "expected_delta_zone_covering_block_count"
                ],
                "expected_delta_exact_byte_count": lane[
                    "expected_delta_exact_byte_count"
                ],
                "largest_expected_delta_exact_block": lane[
                    "largest_expected_delta_exact_block"
                ],
            }
        )
    return {
        "schema": "phoenix-mmi.exact-block-map-disc/v1",
        "analysis_mode": comparison["analysis_mode"],
        "artifact": {
            "sha256": comparison[f"{prefix}_artifact_sha256"],
            "side_ordinal": index,
            "source_path_included": False,
        },
        "search_contract": comparison["search_contract"],
        "lanes": lane_rows,
        "transition_envelope": comparison["transition_envelope"],
        "strict_seed_control": comparison["strict_seed_control"],
        "classification": comparison["classification"],
        "limits": comparison["limits"],
        "publication_safety": comparison["publication_safety"],
    }


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Map exact byte-identical blocks only inside fixed windows "
            "around the two Session 032 reorder zones"
        )
    )
    parser.add_argument("firmware_cd1", type=Path)
    parser.add_argument("firmware_cd3", type=Path)
    parser.add_argument("-o", "--output", type=Path, required=True)
    parser.add_argument("--public-output", type=Path, required=True)
    parser.add_argument("--margin", type=_integer, default=0x20000)
    parser.add_argument("--seed-size", type=_integer, default=64)
    parser.add_argument("--stride", type=_integer, default=4)
    parser.add_argument(
        "--firmware-register",
        type=Path,
        default=Path("research/firmware-5570/manifests/artifacts.csv"),
    )
    parser.add_argument(
        "--session003-root",
        type=Path,
        default=Path("research/firmware-5570/session003"),
    )
    parser.add_argument(
        "--navigation-root",
        type=Path,
        default=Path("research/navigation-media"),
    )
    args = parser.parse_args()

    rows = _register(args.firmware_register)
    summaries = {
        disc: _load_json(
            args.session003_root / f"{disc}-mmi.public-summary.json",
            "phoenix-mmi.public-summary/v1",
        )
        for disc in ("cd1", "cd3")
    }
    prior = _load_json(
        args.navigation_root
        / "session033/cd1-cd3.relocation-descriptors.comparison.json",
        "phoenix-mmi.relocation-descriptor-comparison/v1",
    )
    prior_correlation = _load_json(
        args.navigation_root
        / "session033/relocation-descriptor-correlation.json",
        "phoenix-mmi.relocation-descriptor-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session034-"
    ) as temporary:
        readers = {}
        for disc, iso_path, member_path in (
            ("cd1", args.firmware_cd1, CD1_MEMBER),
            ("cd3", args.firmware_cd3, CD3_MEMBER),
        ):
            row = rows.get(iso_path.name)
            if row is None:
                raise ValueError(
                    f"{iso_path.name} is absent from the firmware register"
                )
            image = ISO9660Image(iso_path)
            _verify_iso(image, row)
            member = image.find_path(member_path)
            extracted = image.extract(
                member,
                Path(temporary) / disc / Path(member.path).name,
            )
            reader = BinaryReader(extracted)
            if reader.sha256() != summaries[disc]["artifact"]["sha256"]:
                raise ValueError(
                    f"{disc} principal-image hash differs from Session 003"
                )
            readers[disc] = reader

        comparison = analyze_exact_block_map(
            readers["cd1"],
            readers["cd3"],
            prior,
            margin=args.margin,
            seed_size=args.seed_size,
            stride=args.stride,
        )
        strict = analyze_exact_block_map(
            readers["cd1"],
            readers["cd3"],
            prior,
            margin=args.margin,
            seed_size=args.seed_size * 2,
            stride=args.stride,
        )
        seed_control = compare_exact_block_seed_control(
            comparison, strict
        )
        comparison["strict_seed_control"] = seed_control
        comparison["classification"]["strict_seed_control"] = (
            seed_control["classification"]
        )

    correlation = correlate_exact_block_map(
        prior_correlation, comparison
    )
    reports = {
        "cd1-exact-block-map.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-exact-block-map.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.exact-block-map.comparison.json": comparison,
        "exact-block-map-correlation.json": correlation,
    }
    for name, report in reports.items():
        write_json(
            report, args.output / name.replace(".public", ".analysis")
        )
        write_json(
            build_public_exact_block_map_report(report),
            args.public_output / name,
        )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
