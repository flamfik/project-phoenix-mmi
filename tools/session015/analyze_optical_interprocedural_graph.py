#!/usr/bin/env python3
"""Build publication-safe Session 015 optical/navigation call-graph reports."""

from __future__ import annotations

import argparse
import csv
import json
from pathlib import Path
import tempfile

from phoenix_mmi.binary import BinaryReader
from phoenix_mmi.iso9660 import ISO9660Image
from phoenix_mmi.optical_callgraph import (
    build_public_optical_callgraph_report,
    compare_optical_navigation_callgraph,
    correlate_optical_sector_model,
)
from phoenix_mmi.report import write_json


CD1_MEMBER = "MMI_HI/MMI/42/DEFAULT/H2_HI_EU.BIN"
CD3_MEMBER = "MMI_HI/MMI/42/DEFAULT/H2_HI_EU_R1006_SH3_AUDIHI_5.BIN"


def _register(path: Path) -> dict[str, dict[str, str]]:
    with path.open(newline="", encoding="utf-8") as handle:
        return {row["artifact"]: row for row in csv.DictReader(handle)}


def _load_json(path: Path, schema: str) -> dict[str, object]:
    report = json.loads(path.read_text(encoding="utf-8"))
    if report.get("schema") != schema:
        raise ValueError(f"unsupported schema in {path}: {report.get('schema')!r}")
    return report


def _verify_iso(image: ISO9660Image, row: dict[str, str]) -> None:
    if image.path.stat().st_size != int(row["size_bytes"]):
        raise ValueError(f"registered size mismatch for {image.path.name}")
    if image.sha256() != row["sha256"].lower():
        raise ValueError(f"registered SHA-256 mismatch for {image.path.name}")


def _disc_report(
    comparison: dict[str, object], *, side: str
) -> dict[str, object]:
    prefix = "left" if side == "cd1" else "right"
    nodes = [
        {
            "domain": node["domain"],
            "depth": node["depth"],
            "entry_file_offset": node[f"{prefix}_entry_file_offset"],
            "known_ratio": node[f"{prefix}_known_ratio"],
            "call_count": node[f"{prefix}_call_count"],
            "unresolved_indirect_call_count": node[
                f"{prefix}_unresolved_indirect_call_count"
            ],
            "function_boundary_asserted": False,
        }
        for node in comparison["graph"]["nodes"]
    ]
    edges = [
        {
            "domain": edge["domain"],
            "depth": edge["depth"],
            "caller_file_offset": edge[f"{prefix}_caller"],
            "callee_file_offset": edge[f"{prefix}_callee"],
            "call_site_offset": edge[f"{prefix}_call_site_offset"],
            "argument_role_classes": {
                register: values[0 if side == "cd1" else 1]
                for register, values in edge["argument_role_classes"].items()
            },
            "paired_argument_constants": {
                register: values[0 if side == "cd1" else 1]
                for register, values in edge["paired_argument_constants"].items()
            },
            "result_forwarded": edge["result_forwarded"],
            "result_dereferenced": edge["result_dereferenced"],
            "sector_size_2048_argument": edge["sector_size_2048_argument"],
            "status": edge["status"],
        }
        for edge in comparison["graph"]["edges"]
    ]
    return {
        "schema": "phoenix-mmi.optical-callgraph-disc/v1",
        "analysis_mode": comparison["analysis_mode"],
        "artifact": {
            "sha256": comparison[f"{prefix}_artifact_sha256"],
            "source_path_included": False,
        },
        "nodes": nodes,
        "edges": edges,
        "classification": {
            "node_count": len(nodes),
            "edge_count": len(edges),
            "sector_read_abi": comparison["classification"]["sector_read_abi"],
            "optical_buffer_owner": "OPEN",
        },
        "publication_safety": comparison["publication_safety"],
    }


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build a depth-bounded cross-version SH call graph from confirmed "
            "navigation targets and code-gated optical record pointers"
        )
    )
    parser.add_argument("firmware_cd1", type=Path)
    parser.add_argument("firmware_cd3", type=Path)
    parser.add_argument("-o", "--output", type=Path, required=True)
    parser.add_argument("--public-output", type=Path, required=True)
    parser.add_argument(
        "--firmware-register",
        type=Path,
        default=Path("research/firmware-5570/manifests/artifacts.csv"),
    )
    parser.add_argument(
        "--session003-root",
        type=Path,
        default=Path("research/firmware-5570/session003"),
    )
    parser.add_argument(
        "--session010-root",
        type=Path,
        default=Path("research/firmware-5570/session010"),
    )
    parser.add_argument(
        "--session014-root",
        type=Path,
        default=Path("research/navigation-media/session014"),
    )
    args = parser.parse_args()

    rows = _register(args.firmware_register)
    summaries = {
        disc: _load_json(
            args.session003_root / f"{disc}-mmi.public-summary.json",
            "phoenix-mmi.public-summary/v1",
        )
        for disc in ("cd1", "cd3")
    }
    contract = _load_json(
        args.session010_root / "cd1-cd3.navigation-dataflow.comparison.json",
        "phoenix-mmi.navigation-dataflow-comparison/v1",
    )
    prior = _load_json(
        args.session014_root / "global-fldb-parser-correlation.json",
        "phoenix-mmi.global-fldb-parser-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="phoenix-mmi-session015-") as temporary:
        readers = {}
        for disc, iso_path, member_path in (
            ("cd1", args.firmware_cd1, CD1_MEMBER),
            ("cd3", args.firmware_cd3, CD3_MEMBER),
        ):
            row = rows.get(iso_path.name)
            if row is None:
                raise ValueError(f"{iso_path.name} is absent from the firmware register")
            image = ISO9660Image(iso_path)
            _verify_iso(image, row)
            member = image.find_path(member_path)
            extracted = image.extract(
                member, Path(temporary) / disc / Path(member.path).name
            )
            reader = BinaryReader(extracted)
            if reader.sha256() != summaries[disc]["artifact"]["sha256"]:
                raise ValueError(f"{disc} principal-image hash differs from Session 003")
            readers[disc] = reader

        comparison = compare_optical_navigation_callgraph(
            readers["cd1"], readers["cd3"], contract
        )

    correlation = correlate_optical_sector_model(prior, comparison)
    correlation["external_context"] = {
        "used_as_local_artifact_evidence": False,
        "sources": [
            {
                "purpose": "Authoritative SH-3 call, delay-slot and addressing semantics",
                "url": (
                    "https://www.renesas.com/en/document/mas/"
                    "sh-3sh-3esh3-dsp-software-manual?language=en"
                ),
            },
            {
                "purpose": "Register argument context used only as ABI guidance",
                "local_reference": "SESSION-013 external technical basis",
            },
        ],
    }
    reports = {
        "cd1-optical-callgraph.public.json": _disc_report(comparison, side="cd1"),
        "cd3-optical-callgraph.public.json": _disc_report(comparison, side="cd3"),
        "cd1-cd3.optical-callgraph.comparison.json": comparison,
        "optical-sector-correlation.json": correlation,
    }
    for name, report in reports.items():
        write_json(report, args.output / name.replace(".public", ".analysis"))
        write_json(
            build_public_optical_callgraph_report(report),
            args.public_output / name,
        )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
