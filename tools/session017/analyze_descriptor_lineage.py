#!/usr/bin/env python3
"""Build publication-safe Session 017 descriptor-lineage reports."""
...
def _disc_report(comparison: dict[str, object], *, side: str) -> dict[str, object]:
    ...
    producer_calls = [
        pair[prefix] for pair in comparison["producer_pairs"]
    ]
    return {
        "schema": "phoenix-mmi.descriptor-producer-lineage-disc/v1",
        ...
        "accessor_occurrence_count": comparison["accessor_family"][
            f"{prefix}_occurrence_count"
        ],
        "static_descriptor_census": comparison["static_descriptor_census"][prefix],
        "field_initializer_census": comparison["field_initializer_census"][prefix],
        "classification": {
            "producer_call_count": len(producer_calls),
            "cross_version_producer_to_accessor_edge": "OPEN",
            "descriptor_initializer": (
                "NOT_FOUND_UNDER_CODE_GATED_MIXED_WIDTH_MODEL"
            ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Trace descriptor-producing calls, field-12 accessor families and "
            "bounded mixed-width field initializers"
        )
    )
    ...
    parser.add_argument(
        "--session016-root",
        type=Path,
        default=Path("research/navigation-media/session016"),
    )
    ...
    dispatch = _load_json(
        args.session016_root / "cd1-cd3.object-dispatch.comparison.json",
        "phoenix-mmi.object-dispatch-context-comparison/v1",
    )
    prior = _load_json(
        args.session016_root / "object-dispatch-correlation.json",
        "phoenix-mmi.object-dispatch-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="phoenix-mmi-session017-") as temporary:
        readers = {}
        ...
        comparison = analyze_descriptor_lineage(
            readers["cd1"], readers["cd3"], dispatch, optical
        )

    correlation = correlate_descriptor_lineage(prior, comparison)
    correlation["external_context"] = {
        "used_as_local_artifact_evidence": False,
        "sources": [
            {
                "purpose": (
                    "Authoritative SH-3 call, delay-slot, displaced load and "
                    "store semantics"
                ),
                "url": (
    ...
    reports = {
        "cd1-descriptor-lineage.public.json": _disc_report(comparison, side="cd1"),
        "cd3-descriptor-lineage.public.json": _disc_report(comparison, side="cd3"),
        "cd1-cd3.descriptor-lineage.comparison.json": comparison,
        "descriptor-lineage-correlation.json": correlation,
    }
    for name, report in reports.items():
        write_json(report, args.output / name.replace(".public", ".analysis"))
        write_json(
            build_public_descriptor_lineage_report(report),
