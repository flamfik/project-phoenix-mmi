#!/usr/bin/env python3
"""Build publication-safe Session 022 owner ingress/state reports."""
...
def _disc_report(
    ...
    return {
        "schema": "phoenix-mmi.owner-ingress-state-disc/v1",
        "analysis_mode": comparison["analysis_mode"],
        "artifact": {
            "sha256": comparison[f"{prefix}_artifact_sha256"],
            "source_path_included": False,
        },
        "owner_ingress": comparison["owner_ingress"][prefix],
        "state_base_profiles": comparison["state_base_profiles"][prefix],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Test bounded static ingress and state-base provenance for the "
            "two Session 021 owner pairs"
        )
    )
    ...
    prior_correlation = _load_json(
        args.session021_root / "linkage-owner-lineage-correlation.json",
        "phoenix-mmi.linkage-owner-lineage-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session022-"
    ) as temporary:
        readers = {}
        ...
        comparison = analyze_owner_ingress_state(
            readers["cd1"], readers["cd3"], prior
        )

    correlation = correlate_owner_ingress_state(
        prior_correlation, comparison
    )
    reports = {
        "cd1-owner-ingress-state.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-owner-ingress-state.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.owner-ingress-state.comparison.json": comparison,
        "owner-ingress-state-correlation.json": correlation,
    }
    for name, report in reports.items():
        write_json(report, args.output / name.replace(".public", ".analysis"))
        write_json(
            build_public_owner_ingress_report(report),
