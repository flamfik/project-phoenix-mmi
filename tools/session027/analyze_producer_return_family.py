#!/usr/bin/env python3
"""Build publication-safe Session 027 producer return-use reports."""
...
def _disc_report(
    ...
    return {
        "schema": "phoenix-mmi.producer-return-family-disc/v1",
        ...
        "producer_target": comparison["producer_target_pair"][
            f"{prefix}_target_file_offset"
        ],
        "pointer_use_census": comparison["pointer_use_census"][prefix],
        "return_flows": [
            {
                "flow_ordinal": row["flow_ordinal"],
                "contract": row[prefix],
                "session025_candidate_family_ordinal": row[
                    "session025_candidate_family_ordinal"
                ],
                "classification": row["classification"],
            }
            for row in comparison["return_flow_pairs"]
        ],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Classify every exact PC-relative use and following return flow "
            "of the Session 026 producer target pair"
        )
    )
    ...
    prior_correlation = _load_json(
        args.session026_root / "call-return-producer-correlation.json",
        "phoenix-mmi.call-return-producer-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session027-"
    ) as temporary:
        readers = {}
        ...
        comparison = analyze_producer_return_family(
            readers["cd1"], readers["cd3"], session026
        )

    correlation = correlate_producer_return_family(
        prior_correlation, comparison
    )
    reports = {
        "cd1-producer-return-family.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-producer-return-family.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.producer-return-family.comparison.json": comparison,
        "producer-return-family-correlation.json": correlation,
    }
    for name, report in reports.items():
        ...
        write_json(
            build_public_producer_return_report(report),
