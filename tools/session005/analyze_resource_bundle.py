#!/usr/bin/env python3
"""Run Session 005 browser-resource bundle analysis from registered ISOs."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run read-only Session 005 browser-resource bundle analysis"
    )
    ...
    with tempfile.TemporaryDirectory(prefix="phoenix-mmi-session005-") as temporary:
        ...
        comparison = compare_resource_bundles(reports[0], reports[1])
        for disc, report in zip(("cd1", "cd3"), reports):
            write_json(report, args.output / f"{disc}-resource-bundle.analysis.json")
            write_json(
                build_public_resource_bundle(report),
                args.public_output / f"{disc}-resource-bundle.public.json",
            )
        write_json(comparison, args.output / "cd1-cd3.resource-bundle.comparison.json")
        write_json(
            comparison,
            args.public_output / "cd1-cd3.resource-bundle.comparison.json",
        )
    return 0
