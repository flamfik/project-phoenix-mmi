#!/usr/bin/env python3
"""Build publication-safe Session 021 residual-owner lineage reports."""
...
def _disc_report(
    ...
    lineage = comparison["residual_lineage"]
    return {
        "schema": "phoenix-mmi.linkage-owner-lineage-disc/v1",
        "analysis_mode": comparison["analysis_mode"],
        "artifact": {
            "sha256": comparison[f"{prefix}_artifact_sha256"],
            "source_path_included": False,
        },
        "global_owner_census": comparison["global_owner_census"][prefix],
        "selected_lineage": {
            "right_residual_target_file_offset": lineage[
                "right_residual_target_file_offset"
            ],
            "right_residual_call_count": lineage[
                "right_residual_call_count"
            ],
            "right_residual_owner_count": lineage[
                "right_residual_owner_count"
            ],
            "dominant_left_target_file_offset": lineage[
                "exact_context_mapping"
            ]["dominant_left_target_file_offset"],
            "left_target_call_count": lineage["left_target_call_count"],
            "left_target_owner_count": lineage["left_target_owner_count"],
            "confirmed_owner_pair_count": lineage[
                "confirmed_owner_pair_count"
            ],
            "probable_owner_pair_count": lineage[
                "probable_owner_pair_count"
            ],
            "aligned_residual_call_count": lineage[
                "aligned_residual_call_count"
            ],
        },
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Group residual CD3 calls into bounded prologue-backed owners, "
            "align them with CD1 owner sequences and census the global family"
        )
    )
    ...
    parser.add_argument(
        "--session020-root",
        type=Path,
        default=Path("research/navigation-media/session020"),
    )
    ...
    prior = _load_json(
        args.session020_root
        / "cd1-cd3.runtime-linkage-family.comparison.json",
        "phoenix-mmi.runtime-linkage-family-comparison/v1",
    )
    prior_correlation = _load_json(
        args.session020_root / "runtime-linkage-family-correlation.json",
        "phoenix-mmi.runtime-linkage-family-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session021-"
    ) as temporary:
        readers = {}
        ...
        comparison = analyze_linkage_owner_lineage(
            readers["cd1"], readers["cd3"], prior
        )

    correlation = correlate_linkage_owner_lineage(
        prior_correlation, comparison
    )
    reports = {
        "cd1-linkage-owner-lineage.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-linkage-owner-lineage.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.linkage-owner-lineage.comparison.json": comparison,
        "linkage-owner-lineage-correlation.json": correlation,
    }
    for name, report in reports.items():
        write_json(report, args.output / name.replace(".public", ".analysis"))
        write_json(
            build_public_linkage_owner_report(report),
