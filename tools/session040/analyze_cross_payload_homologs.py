#!/usr/bin/env python3
"""Build publication-safe Session 040 cross-payload homolog reports."""
...
def _disc_report(
    comparison: dict[str, object], *, disc: str
) -> dict[str, object]:
    return {
        "schema": "phoenix-mmi.cross-payload-homolog-disc/v1",
        "analysis_mode": comparison["analysis_mode"],
        "disc_id": disc,
        "signature_contract": comparison["signature_contract"],
        "corpus_contract": comparison["corpus_contract"],
        "disc_summary": comparison["disc_summaries"][disc],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Search all registered update payloads for the fixed five-anchor "
            "Session 038 component geometry"
        )
    )
    ...
    prior_correlation = _load_json(
        args.navigation_root
        / "session039/registered-provenance-correlation.json",
        "phoenix-mmi.registered-provenance-correlation/v1",
    )
    ...
    with tempfile.TemporaryDirectory(
        ...
        comparison = analyze_cross_payload_homologs(
            readers["cd1"],
            readers["cd3"],
            session038,
            session039,
    ...
    write_json(
        comparison,
        args.output / "cross-payload-homolog.analysis.json",
    )
    public = build_public_cross_payload_homolog_report(comparison)
    reports = {
        "cross-payload-homolog.comparison.json": public,
        "cd1-cross-payload-homolog.public.json": (
            build_public_cross_payload_homolog_report(
                _disc_report(public, disc="cd1")
            )
        ),
        "cd2-cross-payload-homolog.public.json": (
            build_public_cross_payload_homolog_report(
                _disc_report(public, disc="cd2")
            )
        ),
        "cd3-cross-payload-homolog.public.json": (
            build_public_cross_payload_homolog_report(
                _disc_report(public, disc="cd3")
            )
        ),
        "cross-payload-homolog-correlation.json": correlation,
    }
