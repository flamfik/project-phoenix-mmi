#!/usr/bin/env python3
"""Build publication-safe Session 037 fixed 2 KiB micro-island reports."""
...
def _disc_report(
    ...
    if side == "cd1":
        observed_region = {
            "role": "source-overlap",
            "start": contract["overlap_start"],
            "end": contract["overlap_end"],
            "length": contract["overlap_length"],
        }
    else:
        observed_region = {
            "role": "two-fixed-mapped-overlaps",
            "mapping_count": 2,
            "length_each": contract["overlap_length"],
            "mapped_offsets_published": False,
        }
    return {
        "schema": "phoenix-mmi.micro-island-disc/v1",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Profile only the replicated 2 KiB Session 036 RZ-012 "
            "overlap, with RZ-013 as a fixed negative control"
        )
    )
    ...
    prior = _load_json(
        ...
        / "session036/cd1-cd3.content-island-atlas.comparison.json",
        "phoenix-mmi.content-island-atlas/v1",
    )
    prior_correlation = _load_json(
        args.navigation_root
        / "session036/content-island-atlas-correlation.json",
        "phoenix-mmi.content-island-atlas-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session037-"
    ) as temporary:
        readers = {}
        ...
        comparison = analyze_micro_island(
            readers["cd1"], readers["cd3"], prior
        )

    correlation = correlate_micro_island(
        prior_correlation, comparison
    )
    write_json(
        comparison,
        args.output / "cd1-cd3.micro-island.analysis.json",
    )
    public_comparison = build_public_micro_island_report(comparison)
    reports = {
        "cd1-cd3.micro-island.comparison.json": public_comparison,
        "cd1-micro-island.public.json": build_public_micro_island_report(
            _disc_report(public_comparison, side="cd1")
        ),
        "cd3-micro-island.public.json": build_public_micro_island_report(
            _disc_report(public_comparison, side="cd3")
        ),
        "micro-island-correlation.json": correlation,
    }
