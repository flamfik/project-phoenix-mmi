#!/usr/bin/env python3
"""Build publication-safe Session 033 reorder-descriptor reports."""
...
def _disc_report(
    ...
    return {
        "schema": "phoenix-mmi.relocation-descriptor-disc/v1",
        "analysis_mode": comparison["analysis_mode"],
        "artifact": {
            "sha256": comparison[f"{key}_artifact_sha256"],
            "source_path_included": False,
        },
        "reorder_bracket": comparison["reorder_bracket"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Search only the Session 032 reorder zones for coherent "
            "source/destination/length descriptor tables"
        )
    )
    ...
    prior_correlation = _load_json(
        args.navigation_root
        / "session032/relocation-breakpoint-correlation.json",
    ...
    with tempfile.TemporaryDirectory(
        ...
        comparison = analyze_relocation_descriptors(
            readers["cd1"], readers["cd3"], prior
        )

    correlation = correlate_relocation_descriptors(
        prior_correlation, comparison
    )
    reports = {
        "cd1-relocation-descriptors.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-relocation-descriptors.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.relocation-descriptors.comparison.json": comparison,
        "relocation-descriptor-correlation.json": correlation,
    }
    for name, report in reports.items():
        ...
        write_json(
            build_public_relocation_descriptor_report(report),
