#!/usr/bin/env python3
"""Build publication-safe Session 024 owner-caller reports."""
...
def _disc_report(
    ...
    candidates = []
    for candidate in comparison["registered_candidate_contracts"]:
        candidates.append(
            {
                "candidate_ordinal": candidate["candidate_ordinal"],
                "source": candidate["source"],
                "seed_contract": candidate["traced_seed_contract"][prefix],
                "exact_signature_count": candidate[
                    "exact_signature_census"
                ][f"{prefix}_count"],
                "exact_signature_call_sites": candidate[
                    "exact_signature_census"
                ][f"{prefix}_call_sites"],
                "compatibility": candidate["compatibility"],
            }
        )
    return {
        "schema": "phoenix-mmi.owner-caller-compatibility-disc/v1",
        "analysis_mode": comparison["analysis_mode"],
        "artifact": {
            "sha256": comparison[f"{prefix}_artifact_sha256"],
            "source_path_included": False,
        },
        "owner_entry_contracts": comparison["owner_entry_contracts"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Test registered memory-loaded dispatch contracts against "
            "selected owner entry arguments"
        )
    )
    ...
    parser.add_argument(
        "--session023-root",
        type=Path,
        default=Path("research/navigation-media/session023"),
    )
    ...
    prior_correlation = _load_json(
        args.session023_root
        / "internal-continuation-contract-correlation.json",
        "phoenix-mmi.internal-continuation-contract-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session024-"
    ) as temporary:
        readers = {}
        ...
        comparison = analyze_owner_caller_compatibility(
            readers["cd1"],
            readers["cd3"],
            dispatch,
            owner_ingress,
    ...
    correlation = correlate_owner_caller_compatibility(
        prior_correlation, comparison
    )
    reports = {
        "cd1-owner-caller-compatibility.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-owner-caller-compatibility.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.owner-caller-compatibility.comparison.json": comparison,
        "owner-caller-compatibility-correlation.json": correlation,
    }
    for name, report in reports.items():
        write_json(report, args.output / name.replace(".public", ".analysis"))
        write_json(
            build_public_owner_caller_report(report),
