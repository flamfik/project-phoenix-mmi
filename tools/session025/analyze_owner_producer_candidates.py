#!/usr/bin/env python3
"""Build publication-safe Session 025 producer-first candidate reports."""
...
def _disc_report(
    ...
    candidates = []
    for family in comparison["candidate_families"]:
        if not family["argument_compatible_candidate_promoted"]:
            continue
        candidates.append(
            {
                "candidate_family_ordinal": family[
                    "candidate_family_ordinal"
                ],
                "owner_shape_sha256": family["owner_shape_sha256"],
                "call_relative_instruction_index": family[
                    "call_relative_instruction_index"
                ],
                "side_contract": family[prefix],
                "consensus_contract": family["consensus_contract"],
                "classification": family["classification"],
                "selected_owner_target_link_established": False,
            }
        )
    return {
        "schema": "phoenix-mmi.owner-producer-candidate-disc/v1",
        "analysis_mode": comparison["analysis_mode"],
        "artifact": {
            "sha256": comparison[f"{prefix}_artifact_sha256"],
            "source_path_included": False,
        },
        "required_owner_entry_arguments": comparison[
            "required_owner_entry_arguments"
        ],
        "registered_owner_registry": comparison[
            "registered_owner_registry"
        ][prefix],
        "producer_first_census": comparison["producer_first_census"][
            prefix
        ],
        "promoted_structural_candidates": candidates,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Find bilateral producer-first r4/r6 contracts only in "
            "registered shared prologue owner shapes"
        )
    )
    ...
    parser.add_argument(
        "--session021-root",
        type=Path,
        default=Path("research/navigation-media/session021"),
    )
    parser.add_argument(
        "--session024-root",
        type=Path,
        default=Path("research/navigation-media/session024"),
    )
    ...
    session021 = _load_json(
        args.session021_root
        / "cd1-cd3.linkage-owner-lineage.comparison.json",
        "phoenix-mmi.linkage-owner-lineage-comparison/v1",
    )
    session024 = _load_json(
        args.session024_root
        / "cd1-cd3.owner-caller-compatibility.comparison.json",
        "phoenix-mmi.owner-caller-compatibility-comparison/v1",
    )
    prior_correlation = _load_json(
        args.session024_root
        / "owner-caller-compatibility-correlation.json",
        "phoenix-mmi.owner-caller-compatibility-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session025-"
    ) as temporary:
        readers = {}
        ...
        comparison = analyze_owner_producer_candidates(
            readers["cd1"],
            readers["cd3"],
            session021,
            session024,
    ...
    correlation = correlate_owner_producer_candidates(
        prior_correlation, comparison
    )
    reports = {
        "cd1-owner-producer-candidates.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-owner-producer-candidates.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.owner-producer-candidates.comparison.json": comparison,
        "owner-producer-candidate-correlation.json": correlation,
    }
    for name, report in reports.items():
        write_json(report, args.output / name.replace(".public", ".analysis"))
        write_json(
            build_public_owner_producer_report(report),
