#!/usr/bin/env python3
"""Build publication-safe Session 028 handoff and field-60 reports."""
...
def _disc_report(
    ...
    return {
        ...
        "static_handoffs": [
            ...
            for row in comparison["static_handoffs"]
        ],
...
def main() -> int:
    ...
    prior_correlation = _load_json(
        args.session027_root
        / "producer-return-family-correlation.json",
        "phoenix-mmi.producer-return-family-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session028-"
    ) as temporary:
        readers = {}
        ...
        comparison = analyze_handoff_field60(
            readers["cd1"],
            readers["cd3"],
            session021,
            session027,
    ...
    correlation = correlate_handoff_field60(
        prior_correlation, comparison
    )
    reports = {
        "cd1-handoff-field60.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-handoff-field60.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.handoff-field60.comparison.json": comparison,
        "handoff-field60-correlation.json": correlation,
    }
    for name, report in reports.items():
        ...
        write_json(
            build_public_handoff_field60_report(report),
