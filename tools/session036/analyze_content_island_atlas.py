#!/usr/bin/env python3
"""Build publication-safe Session 036 fixed content-island reports."""
...
def _disc_report(
    ...
    return {
        "schema": "phoenix-mmi.content-island-atlas-disc/v1",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build a fixed 4 KiB two-delta content-island atlas only "
            "inside the unchanged Session 034 envelope"
        )
    )
    ...
    parser.add_argument("--tile-size", type=_integer, default=4096)
    parser.add_argument(
        "--firmware-register",
        type=Path,
    ...
    if args.tile_size % 2:
        raise ValueError("tile size must be even for half-tile control")
    ...
    prior = _load_json(
        ...
        / "session035/cd1-cd3.dual-delta-similarity.comparison.json",
        "phoenix-mmi.dual-delta-similarity-comparison/v1",
    )
    prior_correlation = _load_json(
        args.navigation_root
        / "session035/dual-delta-similarity-correlation.json",
        "phoenix-mmi.dual-delta-similarity-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session036-"
    ) as temporary:
        readers = {}
        ...
        shifted = analyze_content_island_atlas(
            ...
            prior,
            tile_size=args.tile_size,
            grid_offset=args.tile_size // 2,
        )
        comparison = finalize_content_island_atlas(
            primary, shifted
        )

    correlation = correlate_content_island_atlas(
        prior_correlation, comparison
    )
    write_json(
        comparison,
        args.output / "cd1-cd3.content-island-atlas.analysis.json",
    )
    write_json(
        shifted,
        args.output / "shifted-grid.content-island-atlas.analysis.json",
    )
    public_comparison = build_public_content_island_atlas_report(
        comparison
    )
    reports = {
        "cd1-cd3.content-island-atlas.comparison.json": public_comparison,
        "cd1-content-island-atlas.public.json": (
            build_public_content_island_atlas_report(
                _disc_report(public_comparison, side="cd1")
            )
        ),
        "cd3-content-island-atlas.public.json": (
            build_public_content_island_atlas_report(
                _disc_report(public_comparison, side="cd3")
            )
        ),
        "content-island-atlas-correlation.json": correlation,
    }
