#!/usr/bin/env python3
"""Build publication-safe Session 019 runtime-slot lineage reports."""
...
def _disc_report(comparison: dict[str, object], *, side: str) -> dict[str, object]:
    ...
    report = {
        "schema": "phoenix-mmi.runtime-slot-lineage-disc/v1",
    ...
    if side == "cd1":
        report["record_run"] = comparison["record_run"]
        report["tail_slot_census"] = comparison["tail_slot_census"]
        report["direct_writer_search"] = comparison["direct_writer_search"]
        report["exact_source_destination_pair_search"] = comparison[
            "exact_source_destination_pair_search"
        ]
    else:
        report["shadow_accessor_cluster"] = comparison["shadow_accessor_cluster"]
    return report
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Trace the CD1 zero-tail call slots, translated static accessor "
            "cluster and bounded direct-writer/relocation models"
        )
    )
    ...
    parser.add_argument(
        "--session018-root",
        type=Path,
        default=Path("research/navigation-media/session018"),
    )
    ...
    prior = _load_json(
        args.session018_root / "cd1-cd3.accessor-dispatch.comparison.json",
        "phoenix-mmi.accessor-call-family-comparison/v1",
    )
    prior_correlation = _load_json(
        args.session018_root / "accessor-dispatch-correlation.json",
        "phoenix-mmi.accessor-dispatch-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="phoenix-mmi-session019-") as temporary:
        readers = {}
        ...
        comparison = analyze_runtime_slot_lineage(
            readers["cd1"], readers["cd3"], prior
        )

    correlation = correlate_runtime_slot_lineage(prior_correlation, comparison)
    correlation["external_context"] = {
        "used_as_local_artifact_evidence": False,
        "sources": [
            {
                "purpose": (
                    "Authoritative SH-3 PC-relative load, branch, store and "
    ...
    reports = {
        "cd1-runtime-slot-lineage.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-shadow-accessor-cluster.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.runtime-slot-lineage.comparison.json": comparison,
        "runtime-slot-lineage-correlation.json": correlation,
    }
    for name, report in reports.items():
        write_json(report, args.output / name.replace(".public", ".analysis"))
        write_json(build_public_runtime_slot_report(report), args.public_output / name)
    return 0
