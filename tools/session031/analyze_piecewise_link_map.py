#!/usr/bin/env python3
"""Build publication-safe Session 031 piecewise link-map reports."""
...
def _disc_report(
    ...
    return {
        "schema": "phoenix-mmi.piecewise-link-map-disc/v1",
        ...
        "unique_pairs": comparison["unique_pairs"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build a bounded relocation-family and piecewise link/file map "
            "from Session 030 literal pools"
        )
    )
    ...
    parser.add_argument(
        "--session030-root",
        type=Path,
        default=Path("research/navigation-media/session030"),
    )
    ...
    with tempfile.TemporaryDirectory(
        ...
        comparison = analyze_piecewise_link_map(
            readers["cd1"],
            readers["cd3"],
            session030,
    ...
    correlation = correlate_piecewise_link_map(
        prior_correlation, comparison
    )
    reports = {
        "cd1-piecewise-link-map.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-piecewise-link-map.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.piecewise-link-map.comparison.json": comparison,
        "piecewise-link-map-correlation.json": correlation,
    }
    for name, report in reports.items():
        ...
        write_json(
            build_public_piecewise_link_map_report(report),
