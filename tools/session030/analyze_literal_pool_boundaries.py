#!/usr/bin/env python3
"""Build publication-safe Session 030 literal-pool boundary reports."""
...
def _disc_report(
    ...
    return {
        "schema": "phoenix-mmi.literal-pool-boundary-disc/v1",
        ...
        "targets": [
            {
                ...
                "profile": row[prefix],
                "pool_end_runtime_callee_established": row[
                    "pool_end_runtime_callee_established"
                ],
            }
            for row in comparison["target_pairs"]
        ],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Test whether registered Session 029 targets lie inside "
            "complete PC-relative literal pools"
        )
    )
    ...
    parser.add_argument(
        "--session029-root",
        type=Path,
        default=Path("research/navigation-media/session029"),
    )
    ...
    session029 = _load_json(
        args.session029_root
        / "cd1-cd3.handoff-mapping.comparison.json",
        "phoenix-mmi.handoff-mapping-comparison/v1",
    )
    prior_correlation = _load_json(
        args.session029_root
        / "handoff-mapping-correlation.json",
        "phoenix-mmi.handoff-mapping-correlation/v1",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    args.public_output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="phoenix-mmi-session030-"
    ) as temporary:
        readers = {}
        ...
        comparison = analyze_literal_pool_boundaries(
            readers["cd1"],
            readers["cd3"],
            session029,
        )

    correlation = correlate_literal_pool_boundaries(
        prior_correlation, comparison
    )
    reports = {
        "cd1-literal-pool-boundary.public.json": _disc_report(
            comparison, side="cd1"
        ),
        "cd3-literal-pool-boundary.public.json": _disc_report(
            comparison, side="cd3"
        ),
        "cd1-cd3.literal-pool-boundary.comparison.json": comparison,
        "literal-pool-boundary-correlation.json": correlation,
    }
    for name, report in reports.items():
        ...
        write_json(
            build_public_literal_pool_boundary_report(report),
